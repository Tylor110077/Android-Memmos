...
> v4.1.3（2026-08-25）面板黄金锚点：「快速抓取」区从滚动流中独立——BoxWithConstraints 取面板
> 可用高度 ×0.618 作锚点，「最近剪藏」滚动区止于锚点上方，按钮顶落在 61.8% 处（实测模拟器
> 2400px 高 → 按钮中心 ≈60.4%）；说明文字随按钮区下沉，滚动区可独立滚动。

> v4.1.4（2026-08-25）浮条设置抽屉收起手势：①内容列去掉 verticalScroll——内部滚动会抢
> 垂直手势，导致滑杆区/下方拖不动；现在**整块抽屉**（含滑杆区、空白区）都可下滑收起；
> ②收起阈值 140dp→**56dp**（短滑即收）+ 900px/s 甩动不变；③只收设置抽屉，面板保持打开
> （sheetOpen 与面板可见性相互独立）。设置行数固定，加行需改十字手势处理。

> v4.1.5（2026-08-25）抽屉收起抽动修复：根因是「AnimatedVisibility 滑入滑出 + 手拖 offset
> 双动画叠加」——松手未过阈值时 sheetDrag 瞬间归零（硬跳回），关闭时与 exit 动画叠层漂移。
> 改为**单一 Animatable 自管理**：滑入(snap 屏下→animateTo 0)、跟手(draggable 映射 snapTo)、
> 回弹(animateTo 0)、滑出(animateTo 屏高→卸载)全部走同一个值，无相位叠加；拖拽开始即 stop
> 进行中的动画。阈值不变（56dp / 900px/s）。

> v4.1.6（2026-08-25）浮条滑杆范围：①不透明度 0.4→1.0 不变，但**玻璃实度**随滑杆联动——
> 填充/描边不透明度 0.18/0.22→1.0，上限=纯白实心（原来只调图层透明度，上限仍是半透玻璃）；
> ②宽度范围 12–36dp → **8–36dp**（可更细、上限不变），示意图与热区（max(48, w+28)）同步适配。

> v4.1.7（2026-08-25）悬浮窗体验一组：
> ①**快速滑滑块闪动**根因=onChange 每次 applyRestingPosition(animated=true)，220ms 窗口动画
> 被每帧 cancel+重启 → 闪一下；改瞬时定位（滑块拖动是连续变化，动画仅用于初始/手动停靠）。
> ②设置抽屉**点空白区域**→整个退出悬浮窗（关抽屉+面板，浮条贴边保留，下次滑开=初始面板）；
> 由抽屉根 Box detectTapGestures 兜底，滑杆/chip/把手各自消费 tap 不触发。
> ③面板/抽屉背景=**90% 纯黑**（0xE6000000），去掉环境背景图与可读性罩（用户要求）。
> ④宽度下限 8→**5**（OverlayModel 钳制曾卡在 12，此前 8 也被顶回——已同步 5/31 映射）。

> v4.1.8（2026-08-25）滑块形态与配色：①形状=**柱状胶囊**（头尾两角全圆 999，去掉原「贴边侧微圆
> 外侧半圆」的不对称收头）；②填充=**纯色统一**（去掉白玻璃渐变；深灰本体加白 25% 描边保轮廓，
> 绿/白不描边）；③**配色三选一** TabColor：GREEN #10B981 / WHITE #F5F7FA / DARK #3A3F4B，
> 设置抽屉新增「配 色」分段选择（真实色点+文字，选中白胶囊黑字），演示示意图同步；默认 GREEN。
> 不透明度滑杆仍作用于图层整体透明（纯色本体 alpha=1 即完全不透明）。

> v4.1.9（2026-08-25）①设置抽屉「顶到顶」修复：自管理动画改造后丢了原来的
> AnimatedVisibility.align(Center) 垂直居中，Box 默认 TopStart → 顶到屏幕顶；
> 改回**贴底对齐**（BottomStart/BottomEnd，0.84 高），下滑收起语义一致。
> ②浮条**静止态低调化**：EdgeTab 新增 active 参数——面板打开或拖拽时透明度 100%，
> 静止（贴边等触发）时透明度乘 0.55，触发器存在感更弱（设置不透明度滑杆仍作用于基准值）。

> v4.1.10（2026-08-26）设置交互重构 + 默认值：①删掉面板头部齿轮按钮，头部下方新增
> 「浮条设置」把手条——**向下滑（≥36dp 或点按）展开顶部内嵌抽屉**，抽屉内**向上滑（≥56dp
> 或甩动）收回**，不再是独立底部大卡片，就在最近剪藏上方像抽屉一样开合；抽屉=面板窗口内
> Compose 位移（单一 Animatable：0↔-0.78×屏高），零窗口几何更新，与面板跟手无冲突。
> ②顶部抽屉样式：底边两角 26dp 圆角、自绘 Path 描边（两侧+底角、顶线不描）。
> ③默认值：浮条宽度 **5dp**、颜色 **白色**、位置 **frac 0.61**。
> ④跟手节流 32ms→**16ms**（帧率回升；闪动方面仍待真机复验——窗口移动在 ColorOS 上的
> 闪烁机理尚未根除，若仍闪下轮考虑纯 Compose 位移替代窗口移动）。
